            ...
            with st.container(border=True):
                ...
                with col1:
                    try:
                        ...
                        stream_id = stream.get('stream_id', f'unknown_{i}')
                        ...
                        st.caption(f"ID: `{stream_id}`")

                        # 显示流状态
                        status = "运行中" if stream.get('status', 'active') == 'active' else "已停止"
                        status_color = "green" if status == "运行中" else "gray"
                ...
                with col2:
                    ...
                    if btn_cols[1].button("⏹️", key=stop_key,
                        ...
                        with st.spinner("停止中..."):
                            try:
                                success, result = client.stop_stream(stream_id)
                                if success:
                                    st.toast(f"视频流 {stream_id} 已停止。", icon="✅")
                                    # 清除当前观看的流如果是同一个
                                    current_stream = st.session_state.app_state['monitoring'].get('viewing_stream_info')
                                    if current_stream and current_stream.get('stream_id') == stream_id:
        ...
        if not success:
            ...
            try:
                ...
                st.dataframe(
                    ...
                    column_order=("image_url", "name", "sn", "similar极_display", "detected_at"),
                    hide_index=True,
                    use_container_width=True,
                    height=500
                )
